...
def main():
    ...
    # the first command is the adaptive timestep
    commands = ["pylith step01a_slip_ts.cfg --nodes=1 --petsc.ts_type=beuler --petsc.ts_adapt_type=basic --petsc.log_view=ascii:./" + log_view_dir + "/1.txt"]

    timestep = 10**np.arange(-1, 1.5, 0.5)

    # timestep = [10]
    print(timestep)
    # different fixed timesteps
    for i in range(len(timestep)):
        commands.append("pylith step01a_slip_ts.cfg --timedependent.initial_dt="+ str(timestep[i]) +"*year --timedependent.start_time=-"+ str(timestep[i]) +"*year --petsc.log_view=ascii:./" + log_view_dir + "/" + str(i+2) +".txt", )
    ...
    with h5py.File(filename, "r") as f:
        ...
        displacement = list(f[vertex_fields])[0]    # data name in the group
        data_tmp = f[vertex_fields][displacement]
        data_baseline = np.squeeze(data_tmp[-1,:,:])    # 2D 
    ...
    timestep = np.hstack((1, timestep))

    np.savez('distance_time_convergence_comparison.npz', t=timestep, error=distance)
